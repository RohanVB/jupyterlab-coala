...
class Linter {
  ...
  async load_linter(){
    ...
    if (!this.app.serviceManager.terminals.isAvailable()) {
      this.log('Disabling jupyterlab-coala plugin because it cant access terminal');
      this.loaded = false;
      this.prefs.toggled = false;
      return;
    }

    this.term = new Terminal({initialCommand: 'echo "Opening coala terminal"'});
    try {
      ...
      setTimeout(() => {
        this.loaded = true;
        this.activate_flake8(); // todo: fix this
      }, 4000)
    }
    catch(e) {
      this.loaded = false;
